...
var donothing ;
// searchPre()
var searchPre2 = function(){
  /**  search functionality **/
  d3.select("#txtpre")
  .on("keyup", function() { // filter according to key pressed 
     var searched_data = window.data,
      text = this.value.trim();
      if(text == ''){window.donothing = "T"}else(window.donothing = "F")
    console.log("TYPE ",text)
    var searchResults = searched_data.map(function(r) {
      var regex = new RegExp("^" + text + ".*", "i");
     ...
     searchResults = searchResults.filter(function(r){ 
      return r != undefined;
         ...
         searched_data = searchResults.map(function(r) {
          ...
          // console.log("searchResults after filet..",p.Name.indexOf(r));
          if(p.Name.indexOf(r) != -1){
            // console.log("VALUE.",p.Height,"---",p.Name);
           ...
           var cName ="";
            for (var k in p) {
              ...
              if( cName != p["Name"]){
                arr.push([p["Name"],p["Height"],p["Weight"]]);
                cName = p["Name"];
              }
            ...
            //____________________________________________
            // return  [[p.Namep,p.Height,p.Date]];
            console.log("Array---",arr);
            console.log("DOING ---",window.donothing);
            if(window.donothing == "T"){
              // Update table  
             d3.select("#presd-table tbody").remove();
             const table = d3.select("#presd-table");
             const tableBody = table.append("tbody");
               tableBody
                 .selectAll("tr")
    ...
    console.log("FILTER DONE ",searchResults);
...
d3.select("#txtpre").on("input", searchPre2 )
